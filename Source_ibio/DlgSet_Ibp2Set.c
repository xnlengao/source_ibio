/*************************************************************************** 
 *	Module Name:	DlgSet_Ibp2Set
 *
 *	Abstract:	设置IBP2配置信息的对话框
 *
 *	Revision History:
 *	Who		When		What
 *	--------	----------	-----------------------------
...
#define  DID_IBP2SET						7500
#define DID_IBP2SET_HELP				DID_IBP2SET
#define DID_IBP2SET_OK 					DID_IBP2SET+1
#define DID_IBP2SET_ALMSOURCE			DID_IBP2SET+2
#define DID_IBP2SET_SET_ALMSOURCE		DID_IBP2SET+3
#define DID_IBP2SET_ALMCLASS			DID_IBP2SET+4
#define DID_IBP2SET_SET_ALMCLASS		DID_IBP2SET+5
#define DID_IBP2SET_ALMPRINT			DID_IBP2SET+6
#define DID_IBP2SET_SET_ALMPRINT		DID_IBP2SET+7
#define DID_IBP2SET_HIGH_SYS			DID_IBP2SET+8
#define DID_IBP2SET_SET_HIGH_SYS		DID_IBP2SET+9
#define DID_IBP2SET_LOW_SYS			DID_IBP2SET+10
#define DID_IBP2SET_SET_LOW_SYS		DID_IBP2SET+11
#define DID_IBP2SET_HIGH_MEAN			DID_IBP2SET+12
#define DID_IBP2SET_SET_HIGH_MEAN		DID_IBP2SET+13
#define DID_IBP2SET_LOW_MEAN			DID_IBP2SET+14
#define DID_IBP2SET_SET_LOW_MEAN		DID_IBP2SET+15
#define DID_IBP2SET_HIGH_DIA			DID_IBP2SET+16
#define DID_IBP2SET_SET_HIGH_DIA		DID_IBP2SET+17
#define DID_IBP2SET_LOW_DIA			DID_IBP2SET+18
#define DID_IBP2SET_SET_LOW_DIA		DID_IBP2SET+19
#define DID_IBP2_SET_CAPTON			DID_IBP2SET+20
...
static WNDPROC OldEditProc;
			...
			//限值
static unsigned char strLimit[100]={0};
			//报警来源
static unsigned char *strAlmSource[] = {"all", "sys", "mean", "dia", "sys_mean", "mean_dia", "sys_dia"};
...
//上下限刻度调整的增量mmHg
#define IBP2_LIMIT_INTER	1

//限值 mmHg
#define IBP2_LIMIT_MAX		IBP_MAX
#define IBP2_LIMIT_MIN		IBP_MIN

//上下限值最小幅度
#define IBP2_LIMITRANGE_MIN 	1
...
static int EditProcHelp(HWND hWnd, int message, WPARAM wParam, LPARAM lParam)
{
	...
	switch(message){
		case MSG_PAINT:{
			id = lParam;
			switch(id){
				case DID_IBP2SET_SET_ALMSOURCE:{
	...
	switch(message){
		...
		case MSG_COMMAND:{
				case MSG_KEYUP:{
					...
					switch(id){
						case DID_IBP2SET_ALMSOURCE:{
							gCfgIbp2.bAlmSource = index;
							SetIntValueToResFile(gFileSetup, "IBP2Setup", "alarm_source",  gCfgIbp2.bAlmSource, 1);
							SetFocus(btnSetAlmSource);
							//改变报警源
							IsAlm_IBP2(TRUE, TRUE);
						}break;
						case DID_IBP2SET_ALMCLASS:{
						...
						case DID_IBP2SET_ALMPRINT:{
							gCfgIbp2.bPrnControl = index;
							SetIntValueToResFile(gFileSetup, "IBP2Setup", "alarm_print",  gCfgIbp2.bPrnControl, 1);
		...
		case MSG_KEYUP:{
			...
			switch(wParam){
				case SCANCODE_CURSORBLOCKDOWN:{
					switch(id){
						case DID_IBP2SET_HIGH_SYS:{
							if(iTmpHigh_Sys <IBP2_LIMIT_MAX){
								iTmpHigh_Sys += IBP2_LIMIT_INTER;
								SetLimitText(gCfgIbp2.bUnit, hWnd, iTmpHigh_Sys);			
							}
						}break;
						case DID_IBP2SET_LOW_SYS:{
							if(iTmpLow_Sys < (iTmpHigh_Sys - IBP2_LIMITRANGE_MIN)){
								iTmpLow_Sys += IBP2_LIMIT_INTER;
								SetLimitText(gCfgIbp2.bUnit, hWnd, iTmpLow_Sys);			
							}
						}break;
						case DID_IBP2SET_HIGH_DIA:{
							if(iTmpHigh_Dia <IBP2_LIMIT_MAX){
								iTmpHigh_Dia += IBP2_LIMIT_INTER;
								SetLimitText(gCfgIbp2.bUnit, hWnd, iTmpHigh_Dia);			
							}
						}break;
						case DID_IBP2SET_LOW_DIA:{
							if(iTmpLow_Dia < (iTmpHigh_Dia - IBP2_LIMITRANGE_MIN)){
								iTmpLow_Dia += IBP2_LIMIT_INTER;
								SetLimitText(gCfgIbp2.bUnit, hWnd, iTmpLow_Dia);			
							}
						}break;
						case DID_IBP2SET_HIGH_MEAN:{
							if(iTmpHigh_Mean <IBP2_LIMIT_MAX){
								iTmpHigh_Mean += IBP2_LIMIT_INTER;
								SetLimitText(gCfgIbp2.bUnit, hWnd, iTmpHigh_Mean);			
							}
						}break;
						case DID_IBP2SET_LOW_MEAN:{
							if(iTmpLow_Mean < (iTmpHigh_Mean - IBP2_LIMITRANGE_MIN)){
								iTmpLow_Mean += IBP2_LIMIT_INTER;
								SetLimitText(gCfgIbp2.bUnit, hWnd, iTmpLow_Mean);			
							}
						}break;
					}
				}break;
				case SCANCODE_CURSORBLOCKUP:{
					switch(id){
						case DID_IBP2SET_HIGH_SYS:{
							if(iTmpHigh_Sys > (iTmpLow_Sys + IBP2_LIMITRANGE_MIN)){
								iTmpHigh_Sys -= IBP2_LIMIT_INTER;
								SetLimitText(gCfgIbp2.bUnit, hWnd, iTmpHigh_Sys);			
							}
						}break;
						case DID_IBP2SET_LOW_SYS:{
							if(iTmpLow_Sys >IBP2_LIMIT_MIN){
								iTmpLow_Sys -= IBP2_LIMIT_INTER;
								SetLimitText(gCfgIbp2.bUnit, hWnd, iTmpLow_Sys);			
							}
						}break;
						case DID_IBP2SET_HIGH_DIA:{
							if(iTmpHigh_Dia > (iTmpLow_Dia + IBP2_LIMITRANGE_MIN)){
								iTmpHigh_Dia -= IBP2_LIMIT_INTER;
								SetLimitText(gCfgIbp2.bUnit, hWnd, iTmpHigh_Dia);			
							}
						}break;
						case DID_IBP2SET_LOW_DIA:{
							if(iTmpLow_Dia>IBP2_LIMIT_MIN){
								iTmpLow_Dia -= IBP2_LIMIT_INTER;
								SetLimitText(gCfgIbp2.bUnit, hWnd, iTmpLow_Dia);			
							}
						}break;
						case DID_IBP2SET_HIGH_MEAN:{
							if(iTmpHigh_Mean > (iTmpLow_Mean + IBP2_LIMITRANGE_MIN)){
								iTmpHigh_Mean -= IBP2_LIMIT_INTER;
								SetLimitText(gCfgIbp2.bUnit, hWnd, iTmpHigh_Mean);			
							}
						}break;
						case DID_IBP2SET_LOW_MEAN:{
							if(iTmpLow_Mean>IBP2_LIMIT_MIN){
								iTmpLow_Mean -= IBP2_LIMIT_INTER;
								SetLimitText(gCfgIbp2.bUnit, hWnd, iTmpLow_Mean);			
							}
						}break;
...
static int InitCtlName(HWND hDlg)
{
			...
			GetStringFromResFiles(gsLanguageRes, "IBP", "caption_ibp_alarm", strMenu, sizeof strMenu,"Calibrate And Zero");	
			...
			memset(strMenu, 0, sizeof strMenu);
			GetStringFromResFiles(gsLanguageRes, "IBP", "low_sys", strMenu, sizeof strMenu,"Low Sys");	
...
static int InitCobList(HWND hDlg)
{
	...
	char strMenu[100];
			...
			SendMessage(cobAlmSource, CB_SETCURSEL, gCfgIbp2.bAlmSource, 0);
			...
			SendMessage(cobAlmClass, CB_ADDSTRING, 0, (LPARAM)strMenu);
			SendMessage(cobAlmClass, CB_SETCURSEL, gCfgIbp2.bAlmControl, 0);
			...
			SendMessage(cobAlmPrint, CB_ADDSTRING, 0, (LPARAM)strMenu);
			SendMessage(cobAlmPrint, CB_SETCURSEL, gCfgIbp2.bPrnControl, 0);
...
{
			//获得限值
			iTmpHigh_Sys = gCfgIbp2.iHigh_Sys;
			iTmpHigh_Dia = gCfgIbp2.iHigh_Dia;
			iTmpHigh_Mean= gCfgIbp2.iHigh_Mean;
			iTmpLow_Sys = gCfgIbp2.iLow_Sys;
			iTmpLow_Dia = gCfgIbp2.iLow_Dia;
			iTmpLow_Mean = gCfgIbp2.iLow_Mean;

			SetLimitText(gCfgIbp2.bUnit, editHigh_Sys, gCfgIbp2.iHigh_Sys);
			SetLimitText(gCfgIbp2.bUnit, editLow_Sys, gCfgIbp2.iLow_Sys);
			SetLimitText(gCfgIbp2.bUnit, editHigh_Mean, gCfgIbp2.iHigh_Mean);			
			SetLimitText(gCfgIbp2.bUnit, editLow_Mean, gCfgIbp2.iLow_Mean);
			SetLimitText(gCfgIbp2.bUnit, editHigh_Dia, gCfgIbp2.iHigh_Dia);
			SetLimitText(gCfgIbp2.bUnit, editLow_Dia, gCfgIbp2.iLow_Dia);
...
/*
	建立对话框
*/
void CreateDlgIbp2Set(HWND hWnd)
{
