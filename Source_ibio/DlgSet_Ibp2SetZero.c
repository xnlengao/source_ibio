...
#define DID_IBP2_ZERO						8200
#define DID_IBP2_ZERO_CAPTION				DID_IBP2_ZERO
#define DID_IBP2_ZERO_ZERO					DID_IBP2_ZERO+1
#define DID_IBP2_ZERO_ADJUSTVAULE			DID_IBP2_ZERO+2
#define DID_IBP2_ZERO_ADJUSTBTN			DID_IBP2_ZERO+3
#define DID_IBP2_ZERO_OK					DID_IBP2_ZERO+4
#define DID_IBP2_ZERO_EDITVIEW			DID_IBP2_ZERO+5
#define DID_IBP2_ZERO_HELP					DID_IBP2_ZERO+6
#define DID_IBP2_ZERO_EDITKP				DID_IBP2_ZERO+7
#define DID_IBP2_ZERO_TIME1				DID_IBP2_ZERO+8
#define DID_IBP2_ZERO_TIME2				DID_IBP2_ZERO+9
#define DID_IBP2_ZERO_INFO1				DID_IBP2_ZERO+10
#define DID_IBP2_ZERO_INFO2				DID_IBP2_ZERO+11
#define DID_IBP2_ZERO_INFO 				DID_IBP2_ZERO+11                        
//帮助对话框
...
static HWND  ibp2whnd;

static S_TIME itime;

static int iTmpTop,iTmpBottom,initvalue,flag_zero=0,flag_cal=0,flagtime=0;
...
int IBP2time()
{
	...
	HDC hdc;
	char strMenu[100];
	if(ibp2whnd==(HWND)NULL) return -1;
		//获得绘图DC
 	hdc = GetClientDC(ibp2whnd);
 	if(hdc == (HDC)NULL) return -1;

		memset(strMenu, 0, sizeof strMenu);
		snprintf(strMenu, sizeof strMenu, "%d %s %d %s %d",  gValueIbp2.sCalTime.wYear,"-"
				,gValueIbp2.sCalTime.bMonth,"-",gValueIbp2.sCalTime.bDay);
...
int IBP2Zeroinfo()
{
	...
	if(ibp2whnd==(HWND)NULL) return -1;
 	...
 	hdc = GetClientDC(ibp2whnd);
 	if(hdc == (HDC)NULL) return -1;
		if(B_PRINTF)printf("gValueIbp2.bZeroResult=%d\n",gValueIbp2.bZeroResult);
...
static int InitCtlName(HWND hDlg)
{
			...
			memset(strMenu, 0, sizeof strMenu);
			GetSysTime(&itime);
			snprintf(strMenu, sizeof strMenu, "%d %s %d %s %d",  itime.wYear,"-"
				,itime.bMonth,"-",itime.bDay);
		...
		case MSG_COMMAND:{
			...
			switch(id){
				...
				case DID_IBP2_ZERO_ADJUSTBTN:{
					flag_cal=1;
					flagtime=1;
					gValueIbp2.Calviewmode=1;
					GetSysTime(&itime);
					iBioCMD_IBP_POLL_STATE(1,1);
					iBioCMD_IBP_CAL2(initvalue-80,&itime);					
					SetFocus(btnadjust);
...
void CreateDlgIbp2Setzero(HWND hWnd)
{
