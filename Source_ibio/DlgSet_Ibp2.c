/*************************************************************************** 
 *	Module Name:	DlgSet_Ibp2
 *
 *	Abstract:	设置IBP2配置信息的主对话框
 *
 *	Revision History:
 *	Who		When		What
 *	--------	----------	-----------------------------
...
#define DID_IBP2					7200
#define DID_IBP2_HELP			DID_IBP2
#define DID_IBP2_OK 			DID_IBP2+1
#define DID_IBP2_LABEL			DID_IBP2+2
#define DID_IBP2_SET_LABEL		DID_IBP2+3
#define DID_IBP2_UNIT			DID_IBP2+4
#define DID_IBP2_SET_UNIT		DID_IBP2+5
#define DID_IBP2_VIEW			DID_IBP2+6
#define DID_IBP2_SET_VIEW		DID_IBP2+7
#define DID_IBP2_SPEED			DID_IBP2+8
#define DID_IBP2_SET_SPEED		DID_IBP2+9
#define DID_IBP2_TOP			DID_IBP2+10
#define DID_IBP2_SET_TOP		DID_IBP2+11
#define DID_IBP2_MIDDLE			DID_IBP2+12
#define DID_IBP2_SET_MIDDLE	DID_IBP2+13
#define DID_IBP2_BOTTOM		DID_IBP2+14
#define DID_IBP2_SET_BOTTOM	DID_IBP2+15
#define DID_IBP2_ZERO			DID_IBP2+16
#define DID_IBP2_SET_ALARM		DID_IBP2+17
#define DID_NEWIBP2_CAPTION	DID_IBP2+18
#define DID_IBP2_FILTER			DID_IBP2+19
#define DID_IBP2_SET_FILTER		DID_IBP2+20
//帮助对话框
...
//波形刻度调整的增量mmHg
#define IBP2_SCALE_INTER 		5

//刻度限值
#define IBP2_SCALE_MAX		IBP_MAX
#define IBP2_SCALE_MIN		IBP_MIN

//波形最小幅度
#define IBP2_RANGE_MIN 		20
...
//显示
static unsigned char *strViewIndex[]={"view_all", "view_mean"};	
...
/*
	...
	5.报警限
*/
static int ChangeIbp2Lable(BYTE bLabel)
{
	switch(bLabel){
		case IBPLAB_ART:
		case IBPLAB_PA:
		...
		case IBPLAB_FAP:{
			//Unit: mmHg 
			gCfgIbp2.bUnit = IBPUNIT_MMHG;
			SendMessage(cobUnit, CB_SETCURSEL, gCfgIbp2.bUnit, 0);
			
			//View Mode: SD(m)
			gCfgIbp2.bViewMode = IBPVIEW_ALL;
			SendMessage(cobView, CB_SETCURSEL, gCfgIbp2.bViewMode, 0);
			//改变显示模式
			ChangeIbp2ViewMode();
			
			//Scale 0~150
			gCfgIbp2.iScaleTop = 150;		
			iTmpScaleTop = gCfgIbp2.iScaleTop;
			SetScaleText(editTop, gCfgIbp2.iScaleTop);
			gCfgIbp2.iScaleBottom = 0;		
			iTmpScaleBottom = gCfgIbp2.iScaleBottom;
			SetScaleText(editBottom, gCfgIbp2.iScaleBottom);
			//改变波形显示
			ChangeWaveRange_Ibp2();
			
			//Alarm Source: 可选
			//Sys Limit 160~90
			gCfgIbp2.iHigh_Sys = 160;
			gCfgIbp2.iLow_Sys = 90;
			
			//Dia Limit 90~50
			gCfgIbp2.iHigh_Dia = 90;
			gCfgIbp2.iLow_Dia = 50;
			
			//Mean Limit 110~70
			gCfgIbp2.iHigh_Mean = 110;
			gCfgIbp2.iLow_Mean = 70;
		}break;
		case IBPLAB_CVP:
		case IBPLAB_LAP:
		case IBPLAB_RAP:
		...
		case IBPLAB_UVP:{
			//Unit: cmH2O
			/*gCfgIbp2.bUnit = IBPUNIT_CMH2O;;*/
			gCfgIbp2.bUnit = IBPUNIT_MMHG;
			SendMessage(cobUnit, CB_SETCURSEL, gCfgIbp2.bUnit, 0);
			
			//View Mode: M(sd)
			gCfgIbp2.bViewMode = IBPVIEW_M;
			SendMessage(cobView, CB_SETCURSEL, gCfgIbp2.bViewMode, 0);
			//改变显示模式
			ChangeIbp2ViewMode();
			
			//Scale 0~40
			gCfgIbp2.iScaleTop = 40;		
			iTmpScaleTop = gCfgIbp2.iScaleTop;
			SetScaleText(editTop, gCfgIbp2.iScaleTop);
			gCfgIbp2.iScaleBottom = 0;		
			iTmpScaleBottom = gCfgIbp2.iScaleBottom;
			...
			//Alarm Source: Mean
			gCfgIbp2.bAlmSource = IBP_ALM_SRC_M;
					
			//Mean Limit 10~0
			gCfgIbp2.iHigh_Mean = 10;
			gCfgIbp2.iLow_Mean = 0;
			
		}break;
	}
	
	//写入配置文件
	SetIntValueToResFile(gFileSetup, "IBP2Setup", "unit",  gCfgIbp2.bUnit, 1);
	SetIntValueToResFile(gFileSetup, "IBP2Setup", "view_mode",  gCfgIbp2.bViewMode, 1);
	SetIntValueToResFile(gFileSetup, "IBP2Setup", "scale_top",  gCfgIbp2.iScaleTop, 4);
	SetIntValueToResFile(gFileSetup, "IBP2Setup", "scale_bottom",  gCfgIbp2.iScaleBottom, 4);
	...
	case MSG_COMMAND:{
		case MSG_KEYUP:{
			if(wParam==SCANCODE_ENTER||CBN_EDITCHANGE==HIWORD(wParam)){
					...
					switch(id){
						case DID_IBP2_LABEL:{
							if(gCfgIbp2.bLabel != index){
								ChangeIbp2Lable(index);
							}
							gCfgIbp2.bLabel = index;
							SetIntValueToResFile(gFileSetup, "IBP2Setup", "label",  gCfgIbp2.bLabel, 1);
							ChangeWaveGain_Ibp2();
							SetFocus(btnSetLabel);
						}break;
						case DID_IBP2_UNIT:{
							gCfgIbp2.bUnit = index;
							SetIntValueToResFile(gFileSetup, "IBP2Setup", "unit",  gCfgIbp2.bUnit, 1);
							SetFocus(btnSetUnit);
						...
						case DID_IBP2_SPEED:{
							gCfgIbp2.bScanSpeed = index+1;
							SetIntValueToResFile(gFileSetup, "IBP2Setup", "scan_speed",  gCfgIbp2.bScanSpeed, 1);
							SetFocus(btnSetSpeed);

							ChangeWaveSpeed_Ibp2();
						}break;
						case DID_IBP2_SET_FILTER:{
							gCfgIbp2.bFilterMode= index;
							SetIntValueToResFile(gFileSetup, "IBP2Setup", "filter_mode",  gCfgIbp2.bFilterMode, 1);
							ChangeFilter_Ibp2();
		...
		case MSG_KEYUP:{
			...
			switch(wParam){
				case SCANCODE_CURSORBLOCKDOWN:{
					switch(id){
						case DID_IBP2_TOP:{
							if(iTmpScaleTop<IBP2_SCALE_MAX){
								iTmpScaleTop += IBP2_SCALE_INTER;
								SetScaleText(hWnd, iTmpScaleTop);			
							}
						}break;
						case DID_IBP2_BOTTOM:{
							if(iTmpScaleBottom < (iTmpScaleTop - IBP2_RANGE_MIN)){
								iTmpScaleBottom += IBP2_SCALE_INTER;
				...
				case SCANCODE_CURSORBLOCKUP:{
					switch(id){
						case DID_IBP2_TOP:{
							if(iTmpScaleTop > (iTmpScaleBottom + IBP2_RANGE_MIN)){
								iTmpScaleTop -= IBP2_SCALE_INTER;
								SetScaleText(hWnd, iTmpScaleTop);			
							}
						}break;
						case DID_IBP2_BOTTOM:{
							if(iTmpScaleBottom >IBP2_SCALE_MIN){
								iTmpScaleBottom -= IBP2_SCALE_INTER;
								SetScaleText(hWnd, iTmpScaleBottom);			
							}
						}break;
				...
				case SCANCODE_ENTER:{
					switch(id){
						case DID_IBP2_TOP:{
							gCfgIbp2.iScaleTop = iTmpScaleTop;
							SetIntValueToResFile(gFileSetup, "IBP2Setup", "scale_top",  gCfgIbp2.iScaleTop, 4);
							SetFocus(btnSetTop);
// 							ChangeWaveRange_Ibp1();
						...
						case DID_IBP2_BOTTOM:{
							gCfgIbp2.iScaleBottom = iTmpScaleBottom;
							SetIntValueToResFile(gFileSetup, "IBP2Setup", "scale_bottom",  gCfgIbp2.iScaleBottom, 4);
...
static int InitCtlName(HWND hDlg)
{
			...
			GetStringFromResFiles(gsLanguageRes, "IBP2", "caption", strMenu, sizeof strMenu,"IBP2");	
			...
			memset(strMenu, 0, sizeof strMenu);
			GetStringFromResFiles(gsLanguageRes, "IBP", "filter", strMenu, sizeof strMenu,"Filter Mode");	
...
static int InitCobList(HWND hDlg)
{
	...
	char strMenu[100];
			...
			SendMessage(cobLabel, CB_SETCURSEL, gCfgIbp2.bLabel, 0);
			...
			memset(strMenu, 0, sizeof strMenu);
			GetStringFromResFiles(gsLanguageRes, "GENERAL", "speed_125", strMenu, sizeof strMenu,"125");	
			...
			SendMessage(cobSpeed, CB_SETCURSEL, gCfgIbp2.bScanSpeed-1, 0);

			memset(strMenu, 0, sizeof strMenu);
			GetStringFromResFiles(gsLanguageRes, "GENERAL", "smoothing_filtering", strMenu, sizeof strMenu,"smoothing filtering");	
			SendMessage(cobFilter, CB_ADDSTRING, 0, (LPARAM)strMenu);
			memset(strMenu, 0, sizeof strMenu);
			GetStringFromResFiles(gsLanguageRes, "GENERAL", "normal_filtering", strMenu, sizeof strMenu,"normal filtering");	
			SendMessage(cobFilter, CB_ADDSTRING, 0, (LPARAM)strMenu);
			memset(strMenu, 0, sizeof strMenu);
			GetStringFromResFiles(gsLanguageRes, "GENERAL", "filter_off", strMenu, sizeof strMenu,"filter off");	
			SendMessage(cobFilter, CB_ADDSTRING, 0, (LPARAM)strMenu);


			SendMessage(cobFilter, CB_SETCURSEL, gCfgIbp2.bFilterMode, 0);
